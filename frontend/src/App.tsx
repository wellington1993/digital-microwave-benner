...
export default function App() {
  ...
  // Monitor de reconexao automatica
  useEffect(() => {
    ...
    const probe = setInterval(async () => {
      try {
        ...
        setIsOffline(false);
        console.log('Servidor restabelecido. Voltando ao modo online.');
      } catch {
        // Continua tentando...
      }
    }, 5000);
  ...
  if (!token && !isOffline) {
    return (
      <div className="min-h-screen bg-zinc-950 flex items-center justify-center p-4">
        <form onSubmit={async (e) => { e.preventDefault(); try { const res = await api.post('/auth/login', { username: 'admin', password: 'admin' }); setToken(res.data.token); sessionStorage.setItem('microwave_token', res.data.token); } catch { setIsOffline(true); } }} className="bg-zinc-900 p-8 rounded-2xl border border-zinc-800 w-full max-w-sm shadow-2xl">
          <h2 className="text-white text-xl font-black text-center uppercase tracking-tighter">Microwave Access</h2>
          <button type="submit" className="w-full mt-8 bg-emerald-600 hover:bg-emerald-500 text-white p-4 rounded-xl font-black transition-all">ENTRAR COMO ADMIN</button>
          <button type="button" onClick={() => setIsOffline(true)} className="w-full mt-6 text-zinc-600 text-[10px] uppercase font-bold tracking-widest">Modo Simulacao</button>
        </form>
      </div>
    );
  }
      ...
      <main className={`bg-zinc-900 p-4 md:p-8 rounded-3xl border-4 flex flex-col md:flex-row gap-8 shadow-2xl transition-all duration-500 ${isOffline ? 'border-amber-900/50' : 'border-zinc-800'}`}>
        ...
        <section className="w-full md:w-72 flex flex-col gap-6">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <button disabled={loading || (state === 'Heating' && isPredefined)} onClick={() => handleAction('start')} className="bg-zinc-800 hover:bg-zinc-700 disabled:opacity-30 p-4 rounded-xl font-bold text-sm transition-all active:scale-95 focus:ring-2 focus:ring-zinc-500 outline-none uppercase">+30s</button>
